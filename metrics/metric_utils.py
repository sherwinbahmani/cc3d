...
def iterate_random_labels(opts, batch_size):
    if False: #opts.G.c_dim == 0:
        ...
        while True:
            ...
            c = [dataset.get_label(np.random.randint(len(dataset)), np.random.randint(dataset.num_samples_per_scene)) for _ in range(batch_size)]
            c = [misc.dict_to_device(all_gen_c_i, opts.device) for all_gen_c_i in c]
            c = misc.stack_tensors_dict(c)
            yield c
