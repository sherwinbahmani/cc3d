...
from metrics_full.metrics import metric_main
from metrics_full.metrics import metric_utils
...
def calc_metrics_for_dataset(ctx, metrics, real_data_path, fake_data_path, mirror, resolution, gpus, verbose, use_cache: bool, num_runs: int):
    dnnlib.util.Logger(should_flush=True)
    print("Real", real_data_path)
    print("Fake", fake_data_path)
    ...
    args.dataset_kwargs = dnnlib.EasyDict(
        class_name='metrics_full.training.dataset.VideoFramesFolderDataset',
        path=real_data_path,
...
@click.option('--use_cache', help='Use stats cache', type=bool, default=False, metavar='BOOL', show_default=True)
