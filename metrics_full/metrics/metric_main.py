...
@register_metric
def fidkitti_full(opts):
    opts.dataset_kwargs.update(max_size=None, xflip=False)
    fid = frechet_inception_distance.compute_fid(opts, max_real=None, num_gen=37691)
    return dict(fidkitti_full=fid)
...
@register_metric
def kidkitti_full(opts):
    opts.dataset_kwargs.update(max_size=None, xflip=False)
    kid = kernel_inception_distance.compute_kid(opts, max_real=37691, num_gen=37691, num_subsets=100, max_subset_size=1000)
    return dict(kidkitti_full=kid)
...
@register_metric
def fvd5515_20f(opts):
    opts.dataset_kwargs.update(max_size=None, xflip=False)
    fvd = frechet_video_distance.compute_fvd(opts, max_real=5515, num_gen=5515, num_frames=20)
    return dict(fvd5515_20f=fvd)
