...
from metrics_full.training.sampling import sample_frames
...
class ImageFolderDataset(Dataset):
    ...
    def _load_raw_image(self, raw_idx):
        ...
        image = resize_crop_img(image, self.resolution)
        return image
...
class VideoFramesFolderDataset(Dataset):
    def __init__(self,
        ...
        **super_kwargs,                                 # Additional arguments for the Dataset base class.
        ...
        # If only one directory in the path and multiple sub directories, search in sub-directory
        sub_dirs = os.listdir(path)
        if len(sub_dirs) == 1:
            sub_dir_path = os.path.join(path, sub_dirs[0])
            if os.path.isdir(sub_dir_path) and len(os.listdir(sub_dir_path)) > 0:
                path = sub_dir_path

        self.sampling_dict = dnnlib.EasyDict(**cfg.sampling)
    ...
    def __getitem__(self, idx: int) -> Dict:
        ...
        frames = resize_crop_img(frames, self.resolution)
...
def resize_crop_img(image, resolution):
    ...
    image = torch.nn.functional.interpolate(image, size=resolution, mode='bilinear', align_corners=False)
    if is_image:
        image = image[0]
    return image.numpy()  # t, c, h, w
...
def video_to_image_dataset_kwargs(video_dataset_kwargs: dnnlib.EasyDict) -> dnnlib.EasyDict:
    ...
    return dnnlib.EasyDict(
        class_name='metrics_full.training.dataset.ImageFolderDataset',
