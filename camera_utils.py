...
class CameraTargetSampler:
    """
    Given camera coordinates and respective target coordinates in [-1, 1]
    Compute extrinsics
    """
    @staticmethod
    def sample(camera_coords, target_coords):
        forward_vectors = math_utils.normalize_vecs(target_coords - camera_coords)
        cam2world = create_cam2world_matrix(forward_vectors, camera_coords)
        look_down_vector = torch.tensor([[ 0., -1.,  0.]], device=forward_vectors.device)
        is_look_down = torch.all(forward_vectors == look_down_vector, dim=1)
        cam2world_look_down = torch.tensor([[[ 1.,  0.,  0.,  0.],
                                             [ 0.,  0., -1.,  1.],
                                             [ 0.,  1.,  0.,  0.],
                                             [ 0.,  0.,  0.,  1.]]], device=cam2world.device)
        cam2world_look_down = cam2world_look_down.repeat(is_look_down.sum(), 1, 1)
        cam2world[is_look_down] = cam2world_look_down
        return cam2world
...
def FOV_to_intrinsics(fov_degrees, device='cpu'):
    ...
    """
    focal_length = (1 / (torch.tan(fov_degrees * 3.14159 / 360) * 2))
    intrinsics = torch.tensor([[1, 0, 0.5], [0, 1, 0.5], [0, 0, 1]], device=device).unsqueeze(0).repeat(fov_degrees.shape[0], 1, 1)
